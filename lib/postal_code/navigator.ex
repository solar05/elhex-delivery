defmodule ElhexDelivery.PostalCode.Navigator do
  ...
  defp format_postal_code(postal_code) do
    error = "Unexpected 'postal_code', recieved (#{inspect(postal_code)})"
  ...
  defp calculate_distance({lat1, long1}, {lat2, long2}) do
    ...
    sin_lat_diff_square = Math.sin(lat_diff / 2) |> Math.pow(2)
    sin_long_diff_square = Math.sin(long_diff / 2) |> Math.pow(2)

    a = sin_lat_diff_square + cos_lat1 * cos_lat2 * sin_long_diff_square
